...
class MD5(MDN):
    ...
    # @staticmethod
    # def round_(round_no):
    #     fun = {1: MD5.f, 2: MD5.g, 3: MD5.h, 4: MD5.i}[round_no]
    #     def operation(A, B, C, D, X, s, i):
    #         return MDN.l_roll((A + fun(B, C, D) + X + MD5.T[i]) & MDN.last32, s)
    #     return operation

    @staticmethod
    def round_1_op(A, B, C, D, X, s, i):
        return (B + MDN.l_roll((A + MD5.f(B, C, D) + X + MD5.T[i]) & MDN.last32, s)) & MDN.last32

    @staticmethod
    def round_2_op(A, B, C, D, X, s, i):
        return (B + MDN.l_roll((A + MD5.g(B, C, D) + X + MD5.T[i]) & MDN.last32, s)) & MDN.last32

    @staticmethod
    def round_3_op(A, B, C, D, X, s, i):
        return (B + MDN.l_roll((A + MD5.h(B, C, D) + X + MD5.T[i]) & MDN.last32, s)) & MDN.last32

    @staticmethod
    def round_4_op(A, B, C, D, X, s, i):
        return (B + MDN.l_roll((A + MD5.i(B, C, D) + X + MD5.T[i]) & MDN.last32, s)) & MDN.last32
    ...
    def _update(self, X) -> bytes:
        ...
        # round 3
        for k in range(4):
            A = MD5.round_3_op(A, B, C, D, X[( 1 - 4 * k) % 16],  4, 32 + 4 * k)
            D = MD5.round_3_op(D, A, B, C, X[( 6 - 4 * k) % 16], 11, 33 + 4 * k)
            C = MD5.round_3_op(C, D, A, B, X[(11 - 4 * k) % 16], 16, 34 + 4 * k)
            B = MD5.round_3_op(B, C, D, A, X[( 0 - 4 * k) % 16], 23, 35 + 4 * k)
